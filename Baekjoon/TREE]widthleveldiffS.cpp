...
//different way of solving width and level problem.
#include <iostream>
#include <algorithm>
#define left _left
#define right _right
...
void inorder(int node, int depth) {
    ...
    //just stores depth and order(index) of each node.
    //calculate the left and right most order of each depth at once later.
    a[node].order = ++order;
    a[node].depth = depth;
